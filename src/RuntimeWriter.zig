//! WARNING: Mostly copied verbatim from `std\io\writer.zig`.
...
pub fn fromBufferedWriter(bwriter: anytype) Self {
    const writeFn = struct {
        fn f(context: *anyopaque, bytes: []const u8) anyerror!usize {
            const T = @TypeOf(bwriter);
            var self = @ptrCast(T, @alignCast(@alignOf(T), context));
            return self.write(bytes);
        }
    }.f;
    return .{ .context = bwriter, .writeFn = writeFn };
}
